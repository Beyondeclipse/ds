...
def analyze_with_deepseek(price_data):
    ...
    【交易指导原则 - 必须遵守】
    1. **技术分析主导** (权重80%)：趋势、支撑阻力、K线形态是主要依据
    2. **市场情绪辅助** (权重10%)：情绪数据用于验证技术信号，不能单独作为交易理由  
    - 情绪与技术同向 → 增强信号信心
    ...
    - 价格突破关键支撑/阻力位是重要信号 
    8. **周期下单指导**:
    - 'sma_5'线与'sma_10'重叠或穿过时，'sma_5'上穿'sma_10'是买入信号（如有做空仓位应平仓），'sma_5'下穿'sma_10'是卖出信号（如有做多仓位应平仓）
    - 'sma_5'如上穿'sma_20'则是强烈买入信号，'sma_5'下穿'sma_20'则是强烈卖出信号
    - 'sma_5'上穿'sma_10'或'sma_20'时，还可通过其斜率判断信号强烈大小，如较强烈可直接平仓后反向操作
    - 'sma_3'参考最近3次值，如存在斜率反转则应进行交易，如斜率向上反转则为看多（如有空单则应平仓），斜率向下则为看空（如有多单则应平仓），此时应考虑进行交易（平仓或加仓）
    - 当无持仓时，应等待'sma_5'上穿或下穿再进行交易，此时应HOLD
    - 当有持仓时，可结合'sma_3'、'sma_5'和'sma_10'的斜率判断信号大小：如持仓为做多，有'sma_3'斜率反转向下应及时平仓；如持仓为做空，有'sma_3'斜率反转向上应及时平仓
    - 当有持仓时，均线趋势比较久，即使有加仓信号，也应该HOLD，避免追涨杀跌
    - 再次强调，交易应尽量看'sma_5'与其它均线是否有交叉或斜率反转，如没有则宁愿等待，不急于一时的交易
    - 这里的'sma_3'、'sma_5'、'sma_10'、'sma_20'看交叉位置、上穿、下穿及斜率反转时应都以最新值3次值来看，而不是整条均线，并且当数值相差大于100则算比较大的差异
